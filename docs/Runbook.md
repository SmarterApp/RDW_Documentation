...
#### Subjects
The subject XML defines subject's attributes for the RDW system. It is tenant's responsibility to define a subject XML based on the [RDW_Subject.xsd.](https://github.com/SmarterApp/RDW_Common/blob/develop/model/src/main/resources/RDW_Subject.xsd) SmarterBalanced's [Math](../deploy/Math_subject.xml) and [ELA](../deploy/ELA_subject.xml) subjects XML are included in this package. Subjects must be loaded into the system before assessment packages. The system allows for subject's updates but only for the data attributes that have not been used by the system at the time of the update.
